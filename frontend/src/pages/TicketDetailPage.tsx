...
export default function TicketDetailPage() {
  ...
  const loadTicket = useCallback(async () => {
    ...
    try {
      ...
      // Load sequential review progress if applicable
      if (ticketData.is_sequential_review && ticketData.sequential_review_chain_id) {
        try {
          const progressData = await ticketApi.getSequentialProgress(ticketId)
          setProgress(progressData)
        } catch (err: any) {
          console.error('Failed to load progress:', err)
          // Don't fail the whole page if progress fails
        }
      }
    } catch (err: any) {
                ...
                {progress ? (
                  <div className="space-y-3">
                    {progress.steps.map((step: any, idx: number) => (
                      <div key={idx} className="flex items-start space-x-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                        <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-semibold ${
                          step.status === 'completed' ? 'bg-green-500 text-white' :
                          step.status === 'in_review' ? 'bg-yellow-500 text-white' :
                          step.status === 'pending' && idx + 1 === progress.current_step ? 'bg-blue-500 text-white' :
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              step.status === 'completed' ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' :
                              step.status === 'in_review' ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200' :
                              'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200'
                            }`}>
                              {step.status === 'completed' ? 'Completed' :
                               step.status === 'in_review' ? 'In Progress' : 'Pending'}
                            </span>
                          </div>
